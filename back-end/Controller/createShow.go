...
// fetch all activity
func FetchAllActivity(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		w.WriteHeader(http.StatusBadRequest)
	...
	startIndex := (page - 1) * recordPerPage
	log.Println("hi")
	allActivities, err := database.FetchAllActivityShowe(Util.NEW_ACTIVITY_COLLECTION, startIndex, recordPerPage)
	log.Println("hi1")
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
	...
	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(status{"status": "success", "data": allActivities})
	return
}
...
func CreateLiveshowController(w http.ResponseWriter, r *http.Request) {
	...
	result, err := database.SaveNewLiveshowData(Util.NEW_LIVESHOW_COLLECTION, liveShow)
	log.Println(result)
...
func CreateActivityController(w http.ResponseWriter, r *http.Request) {
	...
	result, err := database.SaveNewActivityData(Util.NEW_ACTIVITY_COLLECTION, activity)
	log.Println(result)
